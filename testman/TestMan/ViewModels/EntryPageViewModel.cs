...
namespace TestMan.ViewModels
{
	public class EntryPageViewModel : BindableBase
	{
        ...
        string selectedBackgroundFocusColorName;
        public string SelectedBackgroundFocusColorName
        {
            get { return selectedBackgroundFocusColorName; }
            set
            {
                if (selectedBackgroundFocusColorName != value)
                {
                    selectedBackgroundFocusColorName = value;
                    RaisePropertyChanged();
                    RaisePropertyChanged("SelectedBackgroundFocusColor");
                }
            }
        }

        public Color SelectedBackgroundFocusColor
        {
            get
            {
                if (string.IsNullOrWhiteSpace(selectedBackgroundFocusColorName))
                {
                    return Color.Default;
                }
                return (Color)colorTypeConverter.ConvertFromInvariantString(selectedBackgroundFocusColorName);
            }
        }
